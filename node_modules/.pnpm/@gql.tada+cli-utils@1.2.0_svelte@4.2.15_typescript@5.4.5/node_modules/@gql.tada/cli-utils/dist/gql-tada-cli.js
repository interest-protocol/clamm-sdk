...
var h = require("node:url");

var g = require("node:os");
...
function runMachineInternal(e, t, r = !1) {
  ...
  for (var a = 0; a < i.length; ++a) {
    ...
    for (var {node: d, state: f} of n) {
      ...
      if (!D) {
        for (var [M, {to: _, reducer: U}] of p.dynamics) {
  ...
  compile() {
    ...
    for (var s of a) {
      ...
      if (this.arity.leading.length > 0 || !this.arity.proxy) {
        ...
        registerStatic(e, p, k.EndOfInput, I.SuccessNode, [ "setSelectedIndex", M ]);
      ...
      for (var g = 0; g < this.arity.leading.length; ++g) {
        ...
        if (this.arity.trailing.length > 0 || g + 1 !== this.arity.leading.length) {
          registerStatic(e, m, k.EndOfInput, I.ErrorNode, [ "setError", "Not enough positional arguments" ]);
          registerStatic(e, m, k.EndOfPartialInput, I.SuccessNode, [ "setSelectedIndex", this.cliIndex ]);
        }
...
var pe = /([\x1B\x9B][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><])/g;

var stripAnsi = e => e.replace(pe, "");
...
var Fe = !!process.env.GITHUB_ACTIONS;
...
function hint(e) {
  return text([ cmd(oe.Style, de.BrightBlack), `${ve.BottomLeft} `, cmd(oe.Style, de.BrightBlue), `${ge.Info} `, cmd(oe.Style, de.Blue), indent(e, "    ") ]);
}

function errorMessage$1(e) {
  return error([ "\n", cmd(oe.Style, [ de.Red, de.Invert ]), ` ${ge.Warning} Error `, cmd(oe.Style, de.NoInvert), `\n${e.trim()}\n` ]);
}
...
function externalError(e, t) {
  ...
  return error([ "\n", cmd(oe.Style, [ de.Red, de.Invert ]), ` ${ge.Warning} ${r} `, cmd(oe.Style, de.NoInvert), `\n${e.trim()}\n`, cmd(oe.Style, de.BrightBlack), `${ve.BottomLeft} `, indent(n, "  ") ]);
}

function experimentMessage(e) {
  return error([ cmd(oe.Style, [ de.Yellow, de.Bold ]), `${ge.Warning} Note: `, cmd(oe.Style, de.Reset), `${e.trim()}\n\n` ]);
}
...
async function* run$6(e, r) {
  ...
  yield function summary$1(e) {
    var t = text([ cmd(oe.Style, de.BrightGreen), `${ge.Tick} Introspection output was generated successfully\n` ]);
    if (e) {
...
function runningPersisted(e, t) {
  var r = e ? t ? `(${e}/${t})` : `(${e})` : "";
  return pipe(interval(150), map((e => text([ cmd(oe.Style, de.Magenta), me[e % me.length], " ", cmd(oe.Style, de.Foreground), `Scanning files${he.Ellipsis} `, cmd(oe.Style, de.BrightBlack), r ]))));
}
...
async function* run$5(e, r) {
    ...
    for await (var p of c) {
      ...
      if (l += p.warnings.length) {
        ...
        for (var g of p.warnings) {
          h += (m = g, text([ ye, cmd(oe.Style, de.BrightBlack), `${m.line}:${m.col}`, he.Tab, cmd(oe.Style, de.Foreground), indent(m.message.trim(), text([ ye, he.Tab ])), he.Newline ]));
          warningGithub$1(g);
        }
        yield h + "\n";
      }
  ...
  if (l && r.failOnWarn) {
    throw function warningSummary$1(e, t) {
      return error([ cmd(oe.Style, de.Red), `${ge.Cross} ${e} warnings `, cmd(oe.Style, de.BrightBlack), `(${t} documents extracted)\n` ]);
    }(l, E);
  } else {
    if (E) {
      try {
    ...
    yield function infoSummary$2(e, t) {
      var r = "";
      if (e) {
        r += text([ cmd(oe.Style, de.BrightYellow), ge.Warning, ` ${e} warnings\n` ]);
      }
      if (t) {
        r += text([ cmd(oe.Style, de.BrightGreen), `${ge.Tick} Persisted manifest was generated successfully `, cmd(oe.Style, de.BrightBlack), `(${t} documents extracted)\n` ]);
      } else {
        r += text([ cmd(oe.Style, de.Blue), `${ge.Info} No persisted documents were found `, cmd(oe.Style, de.BrightBlack), "(Persisted manifest was not generated)\n" ]);
      }
      return r;
    }(l, E);
  }
...
async function* run$4(r, n) {
  ...
  yield function summary() {
    return text([ cmd(oe.Style, de.BrightGreen), `${ge.Tick} Introspection output was generated successfully\n` ]);
  }();
}
...
async function* run$3(e, r) {
  ...
  try {
    for await (var p of c) {
      ...
      if (p.warnings.length) {
        ...
        for (var g of p.warnings) {
          h += (m = g, text([ $e, cmd(oe.Style, de.BrightBlack), `${m.line}:${m.col}`, he.Tab, cmd(oe.Style, de.Foreground), indent(m.message.trim(), text([ $e, he.Tab ])), he.Newline ]));
          warningGithub(g);
        }
        yield h + "\n";
      }
  ...
  if (l && r.failOnWarn) {
    throw function warningSummary(e, t) {
      return error([ cmd(oe.Style, de.Red), `${ge.Cross} ${e} warnings `, cmd(oe.Style, de.BrightBlack), `(${t} document types cached)\n` ]);
    }(l, E);
  } else {
    try {
    ...
    yield function infoSummary$1(e, t) {
      ...
      if (t) {
        r += text([ cmd(oe.Style, de.BrightGreen), `${ge.Tick} Type cache was generated successfully `, cmd(oe.Style, de.BrightBlack), `(${t} document types cached)\n` ]);
      } else {
        r += text([ cmd(oe.Style, de.Blue), `${ge.Info} No documents were found\n` ]);
      }
      return r;
    }(l, E);
  }
...
function diagnosticMessage(e) {
  ...
  return text([ Te, cmd(oe.Style, de.BrightBlack), `${e.line}:${e.col}`, he.Tab, cmd(oe.Style, t), e.severity, he.Tab, cmd(oe.Style, de.Foreground), indent(e.message.trim(), text([ Te, he.Tab, he.Tab ])), he.Newline ]);
}
...
function runningDiagnostics(e, t) {
  var r = e ? t ? `(${e}/${t})` : `(${e})` : "";
  return pipe(interval(150), map((e => text([ cmd(oe.Style, de.Magenta), me[e % me.length], " ", cmd(oe.Style, de.Foreground), `Checking files${he.Ellipsis} `, cmd(oe.Style, de.BrightBlack), r ]))));
}
...
async function* run$2(e, r) {
  ...
  if (r.failOnWarn && s.warn || s.error) {
    throw function problemsSummary(e) {
      ...
      var i = "";
      if (t) {
        i += text([ cmd(oe.Style, de.Blue), ge.Info, ` ${t} notices\n` ]);
      }
      return error(i += text([ cmd(oe.Style, de.Red), ge.Cross, ` ${r + n} problems (${r} errors, ${n} warnings)\n` ]));
    }(s);
  } else {
    yield function infoSummary(e) {
      ...
      if (r || n) {
        i += text([ cmd(oe.Style, de.BrightYellow), ge.Warning, ` ${r + n} problems (${r} errors, ${n} warnings)\n` ]);
      } else {
        i += text([ cmd(oe.Style, de.BrightGreen), ge.Tick, " No problems found\n" ]);
      }
      return i;
    }(s);
  }
...
function completedTask(e, t = !1) {
  return text([ emptyLine(), cmd(oe.Style, de.BrightBlack), t ? ve.BottomLeft : ve.VerticalRight, " ", cmd(oe.Style, de.Green), ge.TickSwoosh, " ", cmd(oe.Style, de.Foreground), e, "\n" ]);
}

function failedTask(e) {
  return text([ emptyLine(), cmd(oe.Style, de.BrightBlack), ve.BottomLeft, " ", cmd(oe.Style, de.BrightRed), ge.CrossSwoosh, " ", cmd(oe.Style, de.Foreground), e, "\n" ]);
}

function warningTask(e) {
  return text([ emptyLine(), cmd(oe.Style, de.BrightBlack), ve.VerticalRight, " ", cmd(oe.Style, de.BrightYellow), ge.Warning, " ", cmd(oe.Style, de.Foreground), e, "\n" ]);
}

function hintMessage(e) {
  return text([ cmd(oe.Style, de.BrightBlack), `${ve.VerticalRight} `, cmd(oe.Style, de.BrightBlue), `${ge.Info} `, cmd(oe.Style, de.Blue), indent(e, text([ cmd(oe.Style, de.BrightBlack), `${ve.Vertical}   `, cmd(oe.Style, de.Blue) ])) ]);
}
...
async function* run$1() {
  ...
  var p;
  try {
    p = t.parseConfig(f.pluginConfig, f.rootPath);
  } catch (e) {
  ...
  yield function success() {
    return text([ "\n", cmd(oe.Style, [ de.Green, de.Invert ]), " Done ", cmd(oe.Style, de.NoInvert), he.Space, "You are all set and ready to go.\n" ]);
  }();
}
...
var Je = function tD() {
  ...
  for (var [h, g] of p.entries()) {
    if (a += g, Xe.has(g)) {
      var {groups: m} = new RegExp(`(?:\\[(?<code>\\d+)m|\\${Ze}(?<uri>.*))`).exec(p.slice(h).join("")) || {
        groups: {}
      };
      if (void 0 !== m.code) {
        ...
        void 0 !== m.uri && (i = 0 === m.uri.length ? void 0 : m.uri);
      }
    }
...
}(), o = (e, t) => ut ? e : t, ot = o("◆", "*"), st = o("■", "x"), Dt = o("▲", "x"), ct = o("◇", "o"), lt = o("┌", "T"), dt = o("│", "|"), ft = o("└", "—"), pt = o("●", ">"), ht = o("○", " "), y = e => {
  switch (e) {
   case "initial":
   case "active":
    return Ve.cyan(ot);
  ...
  return new xD({
    ...
    render() {
      ...
      switch (this.state) {
       ...
       default:
        return `${n}${Ve.cyan(dt)}  ${this.value ? `${Ve.green(pt)} ${t}` : `${Ve.dim(ht)} ${Ve.dim(t)}`} ${Ve.dim("/")} ${this.value ? `${Ve.dim(ht)} ${Ve.dim(r)}` : `${Ve.green(pt)} ${r}`}\n${Ve.cyan(ft)}\n`;
      }
    }
  }).prompt();
}, ue = (e = "") => {
...
var Tt = function resolveCommand$1(e) {
...
function npmRunPathEnv({env: t = s.env, ...r} = {}) {
  ...
  t[n] = function npmRunPath(t = {}) {
    ...
    var a;
    var D = i instanceof URL ? h.fileURLToPath(i) : i;
    var c = r instanceof URL ? h.fileURLToPath(r) : r;
    var l = e.resolve(c);
    var d = [];
    while (a !== l) {
...
var findSignalByNumber = (e, t) => {
  var r = t.find((({name: t}) => g.constants.signals[t] === e));
  if (void 0 !== r) {
    return r;
  }
...
var makeError = ({stdout: e, stderr: t, all: r, error: n, signal: i, exitCode: a, command: D, escapedCommand: c, timedOut: l, isCanceled: d, killed: f, parsed: {options: {timeout: p, cwd: h = s.cwd()}}}) => {
  var g = void 0 === (i = null === i ? void 0 : i) ? void 0 : rr[i].description;
...
var mr = (async () => {})().constructor.prototype;

var Cr = [ "then", "catch", "finally" ].map((e => [ e, Reflect.getOwnPropertyDescriptor(mr, e) ]));
...
var Er = b.debuglog("execa").enabled;
...
function execa(e, t, r) {
  ...
  var h = onetime((async () => {
    ...
    var g = handleOutput(n.options, d);
    if (e || 0 !== t || null !== r) {
      var m = makeError({
        error: e,
        exitCode: t,
