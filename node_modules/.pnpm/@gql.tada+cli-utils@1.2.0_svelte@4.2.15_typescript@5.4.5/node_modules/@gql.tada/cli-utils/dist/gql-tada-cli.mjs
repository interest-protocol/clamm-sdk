...
import { loadConfig as r, parseConfig as n, load as i, outputIntrospectionFile as a, minifyIntrospection as s, readTSConfigFile as D } from "@gql.tada/internal";
...
import f, { emitKeypressEvents as p } from "node:readline";

import { printSchema as h } from "graphql";

import { parseJsonText as m, convertToObject as g } from "typescript";
    ...
    for (var {node: d, state: f} of n) {
      ...
      if (!r || a < i.length - 1 || h) {
        if (h) {
          ...
          for (var {to: g, reducer: E} of m) {
            l.push({
              node: g,
              state: void 0 !== E ? execute(he, E, f, s, c) : f
            });
        ...
        for (var b of Object.keys(p.statics)) {
          ...
          if (s === b) {
            for (var {to: B, reducer: S} of p.statics[b]) {
              l.push({
                node: B,
                state: void 0 !== S ? execute(he, S, f, s, c) : f
              });
              debug(`      Static transition to ${B} found`);
            }
          } else {
            for (var {to: O} of p.statics[b]) {
              l.push({
                node: O,
                state: {
                  ...f,
      ...
      if (!D) {
        for (var [k, {to: I, reducer: M}] of p.dynamics) {
          if (execute(pe, k, f, s, c)) {
            l.push({
              node: I,
              state: void 0 !== M ? execute(he, M, f, s, c) : f
            });
...
class CommandBuilder {
  ...
  addPositional({name: e = "arg", required: t = !0} = {}) {
    ...
    } else if (this.arity.extra !== ve && 0 === this.arity.extra.length) {
  ...
  compile() {
    ...
    for (var s of a) {
      ...
      if (this.arity.leading.length > 0 || !this.arity.proxy) {
        ...
        registerStatic(e, p, X.EndOfInput, Z.SuccessNode, [ "setSelectedIndex", Q ]);
      ...
      var E = h;
      if (this.arity.extra === ve || this.arity.extra.length > 0) {
        var w = injectNode(e, {
          dynamics: [],
          shortcuts: [],
          statics: {}
...
var me = Symbol("clipanion/errorCommand");
...
class Cli {
  ...
  error(e, t) {
    var r;
    var {colored: n, command: i = (null !== (r = e[me]) && void 0 !== r ? r : null)} = void 0 === t ? {} : t;
...
function cmd(e, t, r) {
  ...
  var n = Be;
  if (e === Fe.SetPrivateMode) {
    n += "?";
    e = Fe.SetMode;
  } else if (e === Fe.UnsetPrivateMode) {
    n += "?";
    e = Fe.UnsetMode;
  }
  if (Array.isArray(t)) {
...
function compose(e) {
  var t = cmd(Fe.Style, [ be.Reset, be.NoInvert ]);
  ...
  return pipe(r, concatMap((e => {
    ...
    return pipe(merge([ pipe(n, takeLast(1), map((e => "string" == typeof e && !e.endsWith("\n") ? "" : e))), n ]), function scan(e, t) {
      ...
      if (t) {
        return cmd(Fe.PrevLine, t) + cmd(Fe.DeleteLines, t + 1);
      } else if (stripAnsi(e)) {
        return cmd(Fe.EraseLine, Ee.Backward) + cmd(Fe.ToColumn, 1);
      } else {
        return "";
      }
...
function initTTY(e = {}) {
  ...
  var s = pipe(function fromReadStream(e) {
    ...
    _setColor(t && a || i);
    if (t) {
      n.write(cmd(Fe.UnsetPrivateMode, we.ShowCursor));
    }
  })), onEnd((function _end() {
    if (t) {
      n.write(cmd(Fe.Reset) + cmd(Fe.ResetPrivateMode) + cmd(Fe.SetPrivateMode, we.ShowCursor) + "\n");
    }
  })), share);
  ...
  return {
    ...
    mode: function mode(...e) {
      if (t) {
        ...
        if (r.length) {
          n.write(cmd(Fe.SetMode, r));
        }
        if (i.length) {
          n.write(cmd(Fe.SetPrivateMode, i));
        }
      }
    },
    modeOff: function modeOff(...e) {
      if (t) {
        ...
        if (r.length) {
          n.write(cmd(Fe.UnsetMode, r));
        }
        if (i.length) {
          n.write(cmd(Fe.UnsetPrivateMode, i));
        }
      }
    }
  };
...
function code(e) {
  return text`${cmd(Fe.Style, be.Underline)}${e}${cmd(Fe.Style, be.NoUnderline)}`;
}

function bold(e) {
  return text`${cmd(Fe.Style, be.Bold)}${e}${cmd(Fe.Style, be.Normal)}`;
}

function hint(e) {
  return text([ cmd(Fe.Style, be.BrightBlack), `${$e.BottomLeft} `, cmd(Fe.Style, be.BrightBlue), `${Ae.Info} `, cmd(Fe.Style, be.Blue), indent(e, "    ") ]);
}

function errorMessage$1(e) {
  return error([ "\n", cmd(Fe.Style, [ be.Red, be.Invert ]), ` ${Ae.Warning} Error `, cmd(Fe.Style, be.NoInvert), `\n${e.trim()}\n` ]);
}
...
function externalError(e, t) {
  ...
  return error([ "\n", cmd(Fe.Style, [ be.Red, be.Invert ]), ` ${Ae.Warning} ${r} `, cmd(Fe.Style, be.NoInvert), `\n${e.trim()}\n`, cmd(Fe.Style, be.BrightBlack), `${$e.BottomLeft} `, indent(n, "  ") ]);
}

function experimentMessage(e) {
  return error([ cmd(Fe.Style, [ be.Yellow, be.Bold ]), `${Ae.Warning} Note: `, cmd(Fe.Style, be.Reset), `${e.trim()}\n\n` ]);
}
...
var writeOutput = async (e, t) => {
  ...
  } else if (!await (n = e, c.stat(n).then((e => e.isFile())).catch((() => !1)))) {
    await c.writeFile(e, t);
  } else {
    var r = e + ".tmp";
    await c.writeFile(r, t);
    try {
      await c.rename(r, e);
    } catch (e) {
      await c.unlink(r);
...
async function* run$6(t, D) {
  ...
  if (!D.output && t.pipeTo) {
    ...
    p = e.resolve(e.dirname(c.configPath), l.tadaOutputLocation);
  } else {
  ...
  try {
    await writeOutput(p, h);
  } catch (e) {
    throw externalError("Something went wrong while writing the introspection file", e);
  }
  yield function summary$1(e) {
    var t = text([ cmd(Fe.Style, be.BrightGreen), `${Ae.Tick} Introspection output was generated successfully\n` ]);
    if (e) {
    ...
    return t;
  }(!D.forceTSFormat && "string" != typeof p);
}
...
function warningFile$1(t) {
  ...
  return text([ cmd(Fe.Style, be.Underline), t, cmd(Fe.Style, be.NoUnderline), "\n" ]);
}
...
function runningPersisted(e, t) {
  var r = e ? t ? `(${e}/${t})` : `(${e})` : "";
  return pipe(interval(150), map((e => text([ cmd(Fe.Style, be.Magenta), Oe[e % Oe.length], " ", cmd(Fe.Style, be.Foreground), `Scanning files${xe.Ellipsis} `, cmd(Fe.Style, be.BrightBlack), r ]))));
}
...
async function* run$5(t, i) {
  ...
  if (!i.output && t.pipeTo) {
    ...
    c = e.resolve(e.dirname(s.configPath), D.tadaPersistedLocation);
  } else {
  ...
  try {
    for await (var m of d) {
      ...
      l = Object.assign(l, m.documents);
      if (f += m.warnings.length) {
        var g = warningFile$1(m.filePath);
        for (var E of m.warnings) {
          g += (w = E, text([ Ne, cmd(Fe.Style, be.BrightBlack), `${w.line}:${w.col}`, xe.Tab, cmd(Fe.Style, be.Foreground), indent(w.message.trim(), text([ Ne, xe.Tab ])), xe.Newline ]));
          warningGithub$1(E);
        }
        yield g + "\n";
      }
      if (t.isInteractive) {
        yield runningPersisted(++h, p);
      }
    }
  } catch (e) {
  ...
  var w;
  var b = Object.keys(l).length;
  if (f && i.failOnWarn) {
    throw function warningSummary$1(e, t) {
      return error([ cmd(Fe.Style, be.Red), `${Ae.Cross} ${e} warnings `, cmd(Fe.Style, be.BrightBlack), `(${t} documents extracted)\n` ]);
    }(f, b);
  } else {
    if (b) {
      try {
        var B = JSON.stringify(l, null, 2);
        await writeOutput(c, B);
      } catch (e) {
        throw externalError("Something went wrong while writing the introspection file", e);
      }
    }
    yield function infoSummary$2(e, t) {
      var r = "";
      if (e) {
        r += text([ cmd(Fe.Style, be.BrightYellow), Ae.Warning, ` ${e} warnings\n` ]);
      }
      if (t) {
        r += text([ cmd(Fe.Style, be.BrightGreen), `${Ae.Tick} Persisted manifest was generated successfully `, cmd(Fe.Style, be.BrightBlack), `(${t} documents extracted)\n` ]);
      } else {
        r += text([ cmd(Fe.Style, be.Blue), `${Ae.Info} No persisted documents were found `, cmd(Fe.Style, be.BrightBlack), "(Persisted manifest was not generated)\n" ]);
      }
      return r;
    }(f, b);
  }
...
async function* run$4(e, a) {
  ...
  var c;
  try {
    c = (await D.load()).schema;
  } catch (e) {
    throw externalError("Failed to load schema.", e);
  }
  var l;
  if (!a.output && e.pipeTo) {
    l = e.pipeTo;
  } else if (a.output) {
    l = t.resolve(process.cwd(), a.output);
  } else {
    var d;
    var f;
    try {
      d = await r(a.tsconfig);
      f = n(d.pluginConfig, d.rootPath);
    } catch (e) {
      throw externalError("Failed to load configuration.", e);
    }
    if ("string" == typeof f.schema && ".graphql" === t.extname(f.schema)) {
  ...
  try {
    await writeOutput(l, h(c));
  } catch (e) {
    throw externalError("Something went wrong while writing the introspection file", e);
  }
  yield function summary() {
    return text([ cmd(Fe.Style, be.BrightGreen), `${Ae.Tick} Introspection output was generated successfully\n` ]);
  }();
}
...
async function* run$3(t, i) {
  ...
  if (!i.output && t.pipeTo) {
    ...
    c = e.resolve(e.dirname(s.configPath), D.tadaTurboLocation);
  } else if (D.tadaOutputLocation) {
    c = e.resolve(e.dirname(s.configPath), D.tadaOutputLocation, "..", "graphql-cache.d.ts");
  ...
  try {
    for await (var m of d) {
      ...
      l = Object.assign(l, m.cache);
      f += m.warnings.length;
      if (m.warnings.length) {
        var g = warningFile(m.filePath);
        for (var E of m.warnings) {
          g += (w = E, text([ Ue, cmd(Fe.Style, be.BrightBlack), `${w.line}:${w.col}`, xe.Tab, cmd(Fe.Style, be.Foreground), indent(w.message.trim(), text([ Ue, xe.Tab ])), xe.Newline ]));
          warningGithub(E);
        }
        yield g + "\n";
      }
      if (t.isInteractive) {
        yield runningTurbo(++h, p);
      }
    }
  } catch (e) {
  ...
  if (f && i.failOnWarn) {
    throw function warningSummary(e, t) {
      return error([ cmd(Fe.Style, be.Red), `${Ae.Cross} ${e} warnings `, cmd(Fe.Style, be.BrightBlack), `(${t} document types cached)\n` ]);
    }(f, b);
  } else {
    try {
      var B = function createCacheContents(e) {
        var t = "";
        for (var r in e) {
          if (t) {
            t += "\n";
    ...
    yield function infoSummary$1(e, t) {
      ...
      if (t) {
        r += text([ cmd(Fe.Style, be.BrightGreen), `${Ae.Tick} Type cache was generated successfully `, cmd(Fe.Style, be.BrightBlack), `(${t} document types cached)\n` ]);
      } else {
        r += text([ cmd(Fe.Style, be.Blue), `${Ae.Info} No documents were found\n` ]);
      }
      return r;
    }(f, b);
  }
...
function diagnosticMessage(e) {
  ...
  return text([ Ve, cmd(Fe.Style, be.BrightBlack), `${e.line}:${e.col}`, xe.Tab, cmd(Fe.Style, t), e.severity, xe.Tab, cmd(Fe.Style, be.Foreground), indent(e.message.trim(), text([ Ve, xe.Tab, xe.Tab ])), xe.Newline ]);
}
...
function runningDiagnostics(e, t) {
  var r = e ? t ? `(${e}/${t})` : `(${e})` : "";
  return pipe(interval(150), map((e => text([ cmd(Fe.Style, be.Magenta), Oe[e % Oe.length], " ", cmd(Fe.Style, be.Foreground), `Checking files${xe.Ellipsis} `, cmd(Fe.Style, be.BrightBlack), r ]))));
}
...
async function* run$2(e, t) {
  ...
  try {
    for await (var p of l) {
      ...
      var h = "";
      for (var m of p.messages) {
        D[m.severity]++;
        if (isMinSeverity(m.severity, c)) {
          h += diagnosticMessage(m);
  ...
  if (t.failOnWarn && D.warn || D.error) {
    throw function problemsSummary(e) {
      ...
      var i = "";
      if (t) {
        i += text([ cmd(Fe.Style, be.Blue), Ae.Info, ` ${t} notices\n` ]);
      }
      return error(i += text([ cmd(Fe.Style, be.Red), Ae.Cross, ` ${r + n} problems (${r} errors, ${n} warnings)\n` ]));
    }(D);
  } else {
    yield function infoSummary(e) {
      ...
      if (r || n) {
        i += text([ cmd(Fe.Style, be.BrightYellow), Ae.Warning, ` ${r + n} problems (${r} errors, ${n} warnings)\n` ]);
      } else {
        i += text([ cmd(Fe.Style, be.BrightGreen), Ae.Tick, " No problems found\n" ]);
      }
      return i;
    }(D);
  }
...
var stat = async (e, t = He.File) => await c.stat(e).then((e => {
  switch (t) {
   case He.File:
    return e.isFile();
...
var findGraphQLConfig = async t => {
  ...
  var s = await (async t => {
    ...
    while (r !== n) {
      ...
      if (await stat(i)) {
        try {
          var a = JSON.parse(await c.readFile(i, "utf8"));
  ...
  if (s) {
    ...
    if (await stat(D)) {
      try {
        var l = JSON.parse(await c.readFile(D, "utf8"));
...
function emptyLine() {
  return text([ cmd(Fe.Style, be.BrightBlack), $e.Vertical, "\n" ]);
}

function completedTask(e, t = !1) {
  return text([ emptyLine(), cmd(Fe.Style, be.BrightBlack), t ? $e.BottomLeft : $e.VerticalRight, " ", cmd(Fe.Style, be.Green), Ae.TickSwoosh, " ", cmd(Fe.Style, be.Foreground), e, "\n" ]);
}

function failedTask(e) {
  return text([ emptyLine(), cmd(Fe.Style, be.BrightBlack), $e.BottomLeft, " ", cmd(Fe.Style, be.BrightRed), Ae.CrossSwoosh, " ", cmd(Fe.Style, be.Foreground), e, "\n" ]);
}

function warningTask(e) {
  return text([ emptyLine(), cmd(Fe.Style, be.BrightBlack), $e.VerticalRight, " ", cmd(Fe.Style, be.BrightYellow), Ae.Warning, " ", cmd(Fe.Style, be.Foreground), e, "\n" ]);
}

function hintMessage(e) {
  return text([ cmd(Fe.Style, be.BrightBlack), `${$e.VerticalRight} `, cmd(Fe.Style, be.BrightBlue), `${Ae.Info} `, cmd(Fe.Style, be.Blue), indent(e, text([ cmd(Fe.Style, be.BrightBlack), `${$e.Vertical}   `, cmd(Fe.Style, be.Blue) ])) ]);
}

function runningTask(e) {
  return pipe(interval(150), map((t => text([ emptyLine(), cmd(Fe.Style, be.Magenta), ke[t % ke.length], " ", cmd(Fe.Style, be.Foreground), e.trim() ]))));
}
...
async function* run$1() {
  yield function title(e, t) {
    var r = text([ cmd(Fe.Style, be.BrightBlack), $e.TopLeft, " ", cmd(Fe.Style, [ be.Magenta, be.Invert ]), ` ${e.trim()} `, cmd(Fe.Style, [ be.NoInvert ]), "\n" ]);
    if (t) {
      r += text([ cmd(Fe.Style, be.BrightBlack), $e.Vertical, ` ${t}\n` ]);
    }
    return r;
  ...
  try {
    var D = t.resolve(a);
    s = JSON.parse(await l.readFile(D, "utf-8"));
  } catch (e) {
  ...
  } else if (!semiverComply(p[1], "1.0.0")) {
    yield failedTask(Ye.CHECK_DEPENDENCIES);
  ...
  var g = i({
    origin: m.schema,
    rootPath: t.dirname(h.configPath)
  });
  try {
    await g.loadIntrospection();
  } catch (e) {
    throw externalError("Failed to load schema.", e);
  }
  ...
  yield function success() {
    return text([ "\n", cmd(Fe.Style, [ be.Green, be.Invert ]), " Done ", cmd(Fe.Style, be.NoInvert), xe.Space, "You are all set and ready to go.\n" ]);
  }();
}
...
var st = function tD() {
  ...
  for (var [h, m] of p.entries()) {
    if (a += m, Dt.has(m)) {
      var {groups: g} = new RegExp(`(?:\\[(?<code>\\d+)m|\\${ct}(?<uri>.*))`).exec(p.slice(h).join("")) || {
        groups: {}
      };
      if (void 0 !== g.code) {
...
var ft = new Map([ [ "k", "up" ], [ "j", "down" ], [ "h", "left" ], [ "l", "right" ] ]), pt = new Set([ "up", "down", "left", "right", "space", "enter" ]);
...
let ht = class x {
  ...
  prompt() {
    ...
    }), f.emitKeypressEvents(this.input, this.rl), this.rl.prompt(), void 0 !== this.opts.initialValue && this._track && this.rl.write(this.opts.initialValue), 
...
var mt = globalThis.process.platform.startsWith("win");
...
}(), o = (e, t) => gt ? e : t, Ct = o("◆", "*"), Ft = o("■", "x"), Et = o("▲", "x"), yt = o("◇", "o"), wt = o("┌", "T"), bt = o("│", "|"), Bt = o("└", "—"), St = o("●", ">"), xt = o("○", " "), y = e => {
  switch (e) {
   case "initial":
   case "active":
    return nt.cyan(Ct);
...
var makeError = ({stdout: e, stderr: t, all: r, error: n, signal: i, exitCode: a, command: s, escapedCommand: D, timedOut: c, isCanceled: l, killed: d, parsed: {options: {timeout: f, cwd: p = E.cwd()}}}) => {
  var h = void 0 === (i = null === i ? void 0 : i) ? void 0 : pr[i].description;
...
var normalizeStdio = e => {
  ...
  if ((e => hr.some((t => void 0 !== e[t])))(e)) {
    throw new Error(`It's not possible to provide \`stdio\` in combination with one of ${hr.map((e => `\`${e}\``)).join(", ")}`);
  }
  if ("string" == typeof t) {
    return t;
  }
  ...
  var r = Math.max(t.length, hr.length);
  return Array.from({
    length: r
  }, ((e, r) => t[r]));
};
...
var mr = Symbol.for("signal-exit emitter");
...
var {onExit: Er} = (yr = processOk(Fr) ? new class SignalExit extends SignalExitBase {
...
var kr = [ "then", "catch", "finally" ].map((e => [ e, Reflect.getOwnPropertyDescriptor(Or, e) ]));
...
function execa(e, t, r) {
  ...
  var p = onetime((async () => {
    ...
    var p = handleOutput(n.options, c);
...
var Nr = (() => {
  ...
  var u = (e = "", t = 0) => {
    ...
    var s = 0 === t ? nt.green(yt) : 1 === t ? nt.red(Ft) : nt.red(Et);
  ...
  process.on("SIGINT", (() => $(1))), process.on("SIGTERM", (() => $(1))), process.on("exit", $), 
  {
    start: (s = "") => {
      i = !0, r = function WD({input: e = w, output: t = b, overwrite: r = !0, hideCursor: n = !0} = {}) {
        var i = d.createInterface({
          input: e,
          output: t,
          prompt: "",
          tabSize: 1
        });
        d.emitKeypressEvents(e, i), e.isTTY && e.setRawMode(!0);
        var C = (n, {name: i}) => {
          if ("" === String(n) && process.exit(0), !r) {
            return;
          }
          d.moveCursor(t, "return" === i ? 0 : -1, "return" === i ? -1 : 0, (() => {
            d.clearLine(t, 1, (() => {
              e.once("keypress", C);
            }));
          }));
        ...
        return n && process.stdout.write(et.cursor.hide), e.once("keypress", C), () => {
          e.off("keypress", C), n && process.stdout.write(et.cursor.show), e.isTTY && !mt && e.setRawMode(!1), 
...
async function run(e) {
  ...
  if (i) {
    ...
    try {
      var a = t.resolve(e, "package.json");
      var s = await l.readFile(a, "utf-8");
      var c = JSON.parse(s);
      if (!c.dependencies) {
        c.dependencies = {};
      }
      ...
      await l.writeFile(a, JSON.stringify(c, null, 2));
  ...
  try {
    var d = t.resolve(e, "tsconfig.json");
    var f = await D(d);
    var p = r.endsWith(".json") || r.endsWith(".graphql");
    f.compilerOptions = {
      ...f.compilerOptions,
      plugins: [ {
        name: "@0no-co/graphqlsp",
        schema: p ? t.relative(e, r) : r,
        tadaOutputLocation: t.relative(e, n)
      } ]
    };
    await l.writeFile(d, JSON.stringify(f, null, 2));
  } catch (e) {}
