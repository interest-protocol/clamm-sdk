...
var parseConfig = (e, r = process.cwd()) => {
  var resolveConfigDir = e => {
    ...
    return l.normalize(e.replace(/\${([^}]+)}/, ((e, t) => {
      if ("configDir" === t) {
        return r;
      } else {
        throw new TadaError(`Substitution "\${${t}}" is not recognized (did you mean 'configDir'?)`);
      }
    })));
  };
  if (e.schema && "object" == typeof e.schema) {
    ...
    if ("headers" in t && t.headers && "object" == typeof t.headers) {
      for (var a in t.headers) {
        if (t.headers[a] && "string" != typeof t.headers[a]) {
          throw new TadaError("Headers at `schema.headers` contain a non-string value at key: " + a);
        }
      }
    } else if ("headers" in t) {
  ...
  return {
    ...n,
    tadaOutputLocation: resolveConfigDir(n.tadaOutputLocation),
    tadaTurboLocation: resolveConfigDir(n.tadaTurboLocation),
    tadaPersistedLocation: resolveConfigDir(n.tadaPersistedLocation)
  };
};
...
export { TSError, TadaError, createProgram, createVirtualHost, findTSConfigFile, getURLConfig, importLib, importModule, load, loadConfig, loadFromSDL, loadFromURL, minifyIntrospectionQuery as minifyIntrospection, outputIntrospectionFile, parseConfig, preprocessIntrospection, readTSConfigFile, resolveModuleFile, resolveTypeScriptRootDir };
