...
var makeIntrospectionQuery = e => {
  if (K && A === e) {
    ...
    return K = _makeIntrospectionQuery(A = e);
  }
};
...
exports.parseConfig = (e, r = process.cwd()) => {
  var resolveConfigDir = e => {
    if (!e) {
      return e;
    }
    return s.normalize(e.replace(/\${([^}]+)}/, ((e, n) => {
      if ("configDir" === n) {
        return r;
      } else {
        throw new TadaError(`Substitution "\${${n}}" is not recognized (did you mean 'configDir'?)`);
      }
    })));
  };
