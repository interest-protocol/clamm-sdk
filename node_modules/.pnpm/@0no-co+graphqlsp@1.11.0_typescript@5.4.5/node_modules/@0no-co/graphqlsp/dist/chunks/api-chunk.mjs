...
function getDiagnostics(s, o = null, l, u, h) {
  ...
  return function validateQuery(s, o = null, l, u) {
    ...
    }(o, s, l, u).flatMap((e => annotations(e, N.Error, "Validation")));
    var f = n(o, s, [ T ]).flatMap((e => annotations(e, N.Warning, "Deprecation")));
    return h.concat(f);
  }(p, o, l, u);
}
...
function advance(e) {
  e.lastIndex = C;
  if (e.test(F)) {
    return F.slice(C, C = e.lastIndex);
  }
}
...
function ignored() {
  for (var e = 0 | F.charCodeAt(C++); 9 === e || 10 === e || 13 === e || 32 === e || 35 === e || 44 === e || 65279 === e; e = 0 | F.charCodeAt(C++)) {
    if (35 === e) {
      while (10 !== (e = F.charCodeAt(C++)) && 13 !== e) {}
    }
  }
  C--;
}
...
function selectionSet() {
  ...
  if (123 === F.charCodeAt(C)) {
    ...
    if (!t.length || 125 !== F.charCodeAt(C++)) {
...
function unrollFragment(e, t) {
  var i = [];
  var r = t.languageService.getDefinitionAtPosition(e.getSourceFile().fileName, e.getStart());
  if (!r || !r.length) {
    return i;
  }
  var [a] = r;
  var n = getSource(t, a.fileName);
  if (!n) {
    return i;
  }
  var s = findNode(n, a.textSpan.start);
  if (!s) {
    return i;
  }
  if (A.isVariableDeclaration(s.parent) && s.parent.initializer && A.isCallExpression(s.parent.initializer)) {
    s = s.parent.initializer;
  } else if (A.isPropertyAssignment(s.parent)) {
    s = s.parent.initializer;
  }
  if (A.isCallExpression(s) && q.has(s.expression.getText())) {
    var [o, l] = s.arguments;
    if (l && A.isArrayLiteralExpression(l)) {
      l.elements.forEach((e => {
        if (A.isIdentifier(e)) {
          i.push(...unrollFragment(e, t));
        }
      }));
    }
    try {
      y(o.getText().slice(1, -1), {
        noLocation: !0
...
function unrollTadaFragments(e, t, i) {
  e.elements.forEach((e => {
    if (A.isIdentifier(e)) {
      ...
      var r = e;
      while (A.isPropertyAccessExpression(r.expression)) {
        r = r.expression;
      }
      if (A.isIdentifier(r.name)) {
...
function findAllCallExpressions(e, t, i = !0) {
  ...
  var n = i ? !1 : !0;
  !function find(i) {
    if (A.isCallExpression(i) && q.has(i.expression.getText())) {
      var [s, o] = i.arguments;
      if (!n && !o) {
        n = !0;
        a.push(...getAllFragments(e.fileName, i, t));
      } else if (o && A.isArrayLiteralExpression(o)) {
        o.elements.forEach((e => {
          if (A.isIdentifier(e)) {
            a.push(...unrollFragment(e, t));
          } else if (A.isPropertyAccessExpression(e)) {
            var i = e;
            while (A.isPropertyAccessExpression(i.expression)) {
              i = i.expression;
            }
            if (A.isIdentifier(i.name)) {
              a.push(...unrollFragment(i.name, t));
            }
          }
        }));
      }
      if (s && A.isNoSubstitutionTemplateLiteral(s)) {
        r.push(s);
      }
      return;
    } else {
      A.forEachChild(i, find);
    }
  }(e);
  return {
    nodes: r,
...
function getAllFragments(e, t, i) {
  var r = [];
  var a = i.languageService.getDefinitionAtPosition(e, t.expression.getStart());
  if (!a || !a.length) {
    return r;
  }
  if (t.arguments[1] && A.isArrayLiteralExpression(t.arguments[1])) {
    t.arguments[1].elements.forEach((e => {
      if (A.isIdentifier(e)) {
...
function resolveTemplate(e, t, i) {
  if (A.isNoSubstitutionTemplateLiteral(e)) {
    return {
...
class LRUCache {
  ...
  * rvalues() {
  ...
  set(e, t, i = {}) {
    ...
    if (void 0 === h) {
      h = 0 === this.#s ? this.#d : 0 !== this.#v.length ? this.#v.pop() : this.#s === this.#t ? this.#V(!1) : this.#s;
      this.#u[h] = e;
      this.#h[h] = t;
      this.#l.set(e, h);
      this.#f[this.#d] = h;
...
function getFragmentsInSource(t, i, r) {
  ...
  n.nodes.filter((e => {
    ...
    var n = resolveTemplate(i, t.fileName, r).combinedText;
    try {
      var s = y(n, {
        noLocation: !0
      });
...
function getPersistedCodeFixAtPosition(e, t, i) {
  if (!(i.config.templateIsCallExpression ?? !0)) {
    return;
  }
  var r = getSource(i, e);
  if (!r) {
  ...
  var n = a;
  if (A.isVariableStatement(n)) {
    n = n.declarationList.declarations.find((e => A.isVariableDeclaration(e) && e.initializer && A.isCallExpression(e.initializer))) || a;
  } else if (A.isVariableDeclarationList(n)) {
    n = n.declarations.find((e => A.isVariableDeclaration(e) && e.initializer && A.isCallExpression(e.initializer))) || a;
  } else if (A.isVariableDeclaration(n) && n.initializer && A.isCallExpression(n.initializer)) {
    n = n.initializer;
  } else {
    while (n && !A.isCallExpression(n)) {
      n = n.parent;
    }
  }
  if (!A.isCallExpression(n) || !(e => {
    var t = e.getText();
    var [i, r] = t.split(".");
    return q.has(i) && "persisted" === r;
  })(n.expression) || !n.typeArguments && !n.arguments[1]) {
    return;
  }
  var s, o = e;
  if (n.typeArguments) {
    var [l] = n.typeArguments;
    if (!A.isTypeQueryNode(l)) {
      return;
    }
    var {node: u, filename: h} = getDocumentReferenceFromTypeQuery(l, e, i);
    s = u;
    o = h;
  } else if (n.arguments[1]) {
    if (!A.isIdentifier(n.arguments[1]) && !A.isCallExpression(n.arguments[1])) {
      return;
    }
    var {node: f, filename: c} = getDocumentReferenceFromDocumentNode(n.arguments[1], e, i);
    s = f;
    o = c;
  }
  if (!s) {
  ...
  var p = s;
  if (!p || !A.isCallExpression(p) || !A.isNoSubstitutionTemplateLiteral(p.arguments[0])) {
    return;
  }
  var d = generateHashForDocument(i, p.arguments[0], o);
  var v = n.arguments[0];
  if (!v) {
    return {
      span: {
      ...
      replacement: `"sha256:${d}")`
    };
  } else if (A.isStringLiteral(v) && v.getText() !== `"sha256:${d}"`) {
    return {
      span: {
        start: v.getStart(),
        length: v.end - v.getStart()
      },
      replacement: `"sha256:${d}"`
    };
  } else if (A.isIdentifier(v)) {
    return {
      span: {
        start: v.getStart(),
        length: v.end - v.getStart()
...
var getDocumentReferenceFromTypeQuery = (e, t, i) => {
  ...
  r.forEach((e => {
    ...
    if (A.isVariableDeclaration(r.parent) && r.parent.initializer && A.isCallExpression(r.parent.initializer) && q.has(r.parent.initializer.expression.getText())) {
...
function getGraphQLDiagnostics(e, t, i) {
  ...
  if (r) {
    ...
    n = function findAllTaggedTemplateNodes(e) {
      var t = [];
      !function find(e) {
        if (A.isTaggedTemplateExpression(e) && q.has(e.tag.getText()) || A.isNoSubstitutionTemplateLiteral(e) && A.isTaggedTemplateExpression(e.parent) && q.has(e.parent.tag.getText())) {
          t.push(e);
          return;
        } else {
          A.forEachChild(e, find);
  ...
  var c = [];
  if (r) {
    var p = findAllPersistedCallExpressions(a).map((t => {
      if (!t.typeArguments && !t.arguments[1]) {
        return {
      ...
      var r, n, s, o, l = e;
      if (t.typeArguments) {
        var [u] = t.typeArguments;
        s = u.getStart();
        o = u.getEnd() - u.getStart();
        if (!A.isTypeQueryNode(u)) {
          return {
            category: A.DiagnosticCategory.Warning,
            code: oe,
            file: a,
        ...
        var {node: h, filename: f} = getDocumentReferenceFromTypeQuery(u, e, i);
        r = h;
        l = f;
        n = u.getText();
      } else if (t.arguments[1]) {
        s = t.arguments[1].getStart();
        o = t.arguments[1].getEnd() - t.arguments[1].getStart();
        if (!A.isIdentifier(t.arguments[1]) && !A.isCallExpression(t.arguments[1])) {
          return {
            category: A.DiagnosticCategory.Warning,
            code: oe,
            file: a,
        ...
        var {node: c, filename: p} = getDocumentReferenceFromDocumentNode(t.arguments[1], e, i);
        r = c;
        l = p;
        n = t.arguments[1].getText();
      }
      if (!r) {
        return {
          category: A.DiagnosticCategory.Warning,
      ...
      var d = r;
      if (!d || !A.isCallExpression(d) || !A.isNoSubstitutionTemplateLiteral(d.arguments[0])) {
        return {
          category: A.DiagnosticCategory.Warning,
          code: ue,
          file: a,
      ...
      if (!t.arguments[0]) {
        return {
          ...
          start: t.arguments.pos,
          length: t.arguments.end - t.arguments.pos
        };
      }
      if (t.arguments[0].getText().slice(1, -1).startsWith("sha256:")) {
        var v = generateHashForDocument(i, d.arguments[0], l);
        if (!v) {
          return null;
        }
        if (`sha256:${v}` !== v) {
...
var runDiagnostics = (t, {nodes: i, fragments: r}, a, n) => {
  ...
  var l = i.map((t => {
    ...
    var g = getDiagnostics(l, a.current, void 0, void 0, d).filter((e => {
    ...
    try {
      ...
      if (m.definitions.some((t => t.kind === e.OPERATION_DEFINITION))) {
        if (!m.definitions.find((t => t.kind === e.OPERATION_DEFINITION)).name) {
          g.push({
...
export { fe as A, CharacterStream as C, bubbleUpTemplate as a, bubbleUpCallExpression as b, q as c, getAllFragments as d, getGraphQLDiagnostics as e, findNode as f, getSource as g, getPersistedCodeFixAtPosition as h, init as i, findAllPersistedCallExpressions as j, getDocumentReferenceFromTypeQuery as k, getDocumentReferenceFromDocumentNode as l, onlineParser as o, print as p, resolveTemplate as r, A as t, unrollTadaFragments as u };
